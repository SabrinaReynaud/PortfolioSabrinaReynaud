// Renvoie un tableau avec les pokémon de l'évolution id
function get_evo(data, id){
  let pokemons = []
  for (const pokemon of data){
    if (id=='Stade 1'){
      if (pokemon["nom"]==pokemon["evolutions"][0]){
        pokemons.push(pokemon)
      }
    }
    else if (id=='Stade 2'){
      if (pokemon["nom"]==pokemon["evolutions"][1]){
        pokemons.push(pokemon)
      }
    }
    else if (id=='Stade 3'){
      if (pokemon["nom"]==pokemon["evolutions"][2]){
        pokemons.push(pokemon)
      }
    }
  }
...
const pokemons = get_evo(data, id)
