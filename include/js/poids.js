// Renvoie un tableau avec les pokémon du poids id
function get_poids(data, id){
  let pokemons = []
  for (const pokemon of data){
    poids=pokemon["poids"].substring(0,pokemon["poids"].length-3)
    poids.replace(",",".")
    if (id=='Léger'){
      ...
      if (parseFloat(poids)<60 && parseFloat(poids)>=10){
        pokemons.push(pokemon)
      }
    }
    else if (id=='Lourd'){
...
const pokemons = get_poids(data, id)
